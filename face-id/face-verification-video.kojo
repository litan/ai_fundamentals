...
// video file to do face verification on
val videoFileName = s"${kojoCtx.baseDir}/face.mp4"
...
val videoFile = new File(videoFileName)
require(
    videoFile.exists,
    s"Cannot find video file: $videoFile")
    ...
    try {
        ...
        while (true) {
            ...
            if (frame != null) {
                ...
                val imageMat0 = converterToMat.convert(frame)
                if (imageMat0 != null) { 
                    val size = imageMat0.size
                    val ar = size.width.toDouble / size.height
                    val width2 = (cb.width * 0.75).toInt
                    val height2 = (width2 / ar).toInt
                    val imageMat = new Mat(width2, height2, imageMat0.`type`)
                    resize(imageMat0, imageMat, new Size(width2, height2))
                ...
                pause(0) // allow thread to be interrupted
            }
        }
    }
    catch {
        case t => // eat up interruption
...
val onMac = net.kogics.kojo.util.Utils.isMac
val grabber =
    if (onMac) new OpenCVFrameGrabber(videoFile)
    else new FFmpegFrameGrabber(videoFile)

Utils.runAsyncMonitored {
    detectSequence(grabber)
    println("Async thread done")
}
